...
struct lentry locktab[NLOCKS]; // declaring the lock table
int nextlock;	//next slot in locks.
...
for(i=0;i<NLOCKS;i++){
		...
		for(j=0;j<NLOCKS;j++){
			lptr->processforlock[j]=0;
			lptr->waittype[j]=0;	
			lptr->prioritytype[j]=0;
		}
		//lptr->cqtail = 1 + ((lptr->cqhead) = newqueue());
