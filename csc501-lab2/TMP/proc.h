...
struct	pentry	{
	...
	int pinh;			//inherited priority of process. -1 of not assigned.
	...
	int	pwaitret;
	int callfromkill;
	int waitonlock;		//on 	which lock process is waiting.
	int acqlocks[50];	//represents the locks which are acquired by the current process.
	unsigned long timeinqueue; //represents the time it takes for a process to wait in the queue.
	int lockretvalue; //returning value of lock, OK or DELETE.
